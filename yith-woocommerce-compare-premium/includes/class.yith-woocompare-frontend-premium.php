...
if( ! class_exists( 'YITH_Woocompare_Frontend_Premium' ) ) {
    ...
    class YITH_Woocompare_Frontend_Premium extends YITH_Woocompare_Frontend {
        ...
        public function __construct() {
	        ...
	        add_action( 'yith_woocompare_before_main_table', array( $this, 'add_clear_all_button' ), 30 );
	    ...
	    public function init_variables(){
		    ...
		    // related enabled
		    $this->related_enabled = get_option( 'yith-woocompare-show-related' ) == 'yes';

		    // enabled share
		    $this->share_enabled = get_option( 'yith-woocompare-enable-share' ) == 'yes';
		    ...
		    foreach ( $products as $product_id ) {
			    ...
			    foreach ( $fields as $field => $name )  {
				    ...
				    switch( $field ) {
					    ...
					    case 'description':
						    // get description
						    $description = ( get_option( 'yith_woocompare_use_full_description' ) == 'yes' ) ? yit_get_prop( $product, 'post_content' ) : '';
	    ...
	    protected function _vars( $products = array() ) {
		    ...
		    $vars = array(
			    ...
			    'show_request_quote' => get_option( 'yith_woocompare_fields_product_info_request_quote', 'yes' ),
        ...
        public function related_product() {
	        ...
	        if( ! $this->related_enabled || empty( $this->products_list ) || isset( $_REQUEST['yith_compare_prod'] ) ) {
		        return;
	        }
		...
		public function add_styles_themes_and_custom(){
			...
			echo '<style>' . yith_woocompare_user_style() . '</style>';
	    ...
	    public function premium_add_product_action_json( $json ){
		    $json['only_one'] = ( get_option( 'yith_woocompare_open_after_second' ) == 'yes' && count( $this->products_list ) <= 1 );
	     ...
	     * @author Francesco Licandro
	     */
	    public function add_clear_all_button(){

	    	if( get_option( 'yith_woocompare_show_clear_all_table', 'no' ) != 'yes' ) {
	    		return;
			}

			$label = get_option( 'yith_woocompare_label_clear_all_table' );

		    $html = '<div class="compare-table-clear">';
			$html .= '<a href="' . $this->remove_product_url( 'all' ) . '" data-product_id="all" class="button yith_woocompare_clear" rel="nofollow">' . $label . '</a>';
		    $html .= '</div>';

		    echo apply_filters( 'yith_woocompare_table_clear_all', $html );
		}
	    ...
	    public function counter_compare_shortcode( $atts ) {
            ...
            ! $args['text'] && $args['text'] = _n( '{{count}}', '{{count}}', $c, 'yith-woocommerce-compare' );
            // add count in text
        ...
        public function add_error_limit_message(){
            ...
            echo '<div class="yith-woocompare-error"><p>'.$message.'</p></div>';
        }
    }
}